...
public sealed class WatchCommandTests
{
    ...
    private readonly WatchCommand _sut;
    ...
    public WatchCommandTests()
    {
        ...
        _sut = new WatchCommand(_binaryManager, _binaryProcessFactory);
    }
    ...
    public async Task HandleAsync_Success(bool minify, string? tailwindVersion)
    {
        ...
        _binaryProcessFactory.Create(binaryFilePath, input, output, minify, true).Returns(process);
        ...
        _binaryProcessFactory.Received(1).Create(binaryFilePath, input, output, minify, true);
